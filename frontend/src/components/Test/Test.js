...
{show&&<ul><input onChange={(e)=>{
    if(e.target.value){
        update.firstName=e.target.value
    }
}} type='text' placeholder='First Name'/></ul>}
</ul><br/>
...
}} type='text' placeholder='Last Name'/></ul>}
</ul><br/>
    <ul>Email : <ul style={{fontWeight:"bolder"}}>{user.email}</ul>
</ul><br/>
    <ul>PhoneNumber : <ul style={{fontWeight:"bolder"}}>+{user.phoneNumber}</ul>
{show&&<ul><input onChange={(e)=>{
    if(e.target.value){
        update.phoneNumber=e.target.value
    }
}} type='text' placeholder='Phone Number'/></ul>}
</ul><br/><br/>
{show&&<Button onClick={()=>{
    updateUser()
    setShow(false)
...
<Card style={{height:"100%",width:"80%"}} className="bg-dark text-white">
      <Card.Img src={url?url:logo} alt="Card image" />
      
    </Card>
 
      <div style={{display:"flex",flexDirection:"column",width:"100%", alignItems:"center"}}> <input type="file" onChange= {(e)=> setImage(e.target.files[0])}></input>
    <button onClick={uploadImage}>Upload</button></div></div>
